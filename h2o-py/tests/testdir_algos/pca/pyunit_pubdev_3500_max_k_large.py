...
def pca_max_k():
    ...
    correctEigNum = pcaRandomized.full_parameters["k"]["actual_value"]
    randomizedNum = len(pcaRandomized._model_json["output"]["importance"].cell_values[0]) - 1
    glrmNum = len(pcaGLRM._model_json["output"]["importance"].cell_values[0]) - 1
    assert correctEigNum == randomizedNum, "PCA Randomized FAIL: expected number of eigenvalues: " + \
                                           correctEigNum + ", actual: " + randomizedNum + "."
